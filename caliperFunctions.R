# R Script to call caliper functions for all sensors
...
measurementCalipersFn <- function(uniqueExams=uniqueExams,
                                    saveCSV=TRUE,
                                    makeDF=FALSE, 
                                    transpose=FALSE) {
  # R Script to call caliper functions for all sensors
    ...
    for (j in 1:length(uniqueSeries)) {
      ...
      if(length(uniqueCharts) > 0) {
        ...
        for (k in 1:length(uniqueCharts)) {
          ...
          # respiration rate caliper is computed during signal
