...
writeNCCAASCIIFn <- function(thisChartDF=thisChartDF, 
  ...
  
  {
    ...
    answerDistance[fixThese] <- 
      sample(c(20:60), size=length(fixThese), replace=TRUE)
    newAnswerRows <- 
    ...
    # sample a new distance from .5 to 2 seconds.
    answerDistance[fixThese] <- 
      sample(c(15:60), size=length(fixThese), replace=TRUE)
    newAnswerRows <- 
  ...
  
  {
    ...
    for(n in 1:(length(examDAT)-1)) {
      examDAT[(n+1)] <- paste( str_pad(n, 6, side="left"),
                               ...
                               # paste0(str_pad(as.integer(thisChartDF$PLE1[n]), 8, side="left"), ".0")
      )
    }
