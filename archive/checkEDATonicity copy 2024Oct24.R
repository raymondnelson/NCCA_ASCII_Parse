...
EDAMvtArtifactFn <- function(tsData=chartDF$c_AutoEDA, preSec=4, zCut=3) {
  ...
  negOut <- outputVector
  {
    ...
    for (i in 1:(length(tsData)-(preLen+postLen)+1)) {
      ...
      if((postDiff)[1]-(postDiff)[2]==0) stop() # next()
      preDiffMean <- mean(preDiff)
      preDiffSD <- sdp(preDiff)
      postDiffMean <- mean(postDiff)
      ...
      # if( ( ( postDiffMean - preDiffMean ) / preDiffSD ) <= -zCut ) { 
      #   # mark the artifact after the end of the preLen
      #   negOut[(i+preLen+postLen-1)] <- "Artifact"
      # } 
      negOut[(i+preLen-1)] <- ( ( tsData[(i+preLen-1)] - preDiffMean ) / preDiffSD )
    } # end for loop
    # which(negOut == "Artifact")
    # which(negOut != 0)
    negOut[which(negOut >= -zCut)] <- 0
    negOut[which(negOut <= -zCut)] <- "Artifact"
    # which(negOut!= "0")
  }
...
EDAPrestimActivityFn <- function(tsData, prestimRows, newPrestimRow, asc=2, dsc=2) {
  ...
  # output is a vector of "0" and "Artifact" codes
  return(prestimVc)
...
checkEDATonicityFn <- function(segmentName,
  ...
  
  {
    ...
    # assign("prestim", prestim, envir=.GlobalEnv)
  }
  ...
  
  {
    ...
    edaMvtVc <- EDAMvtArtifactFn(tsData=tsData)
