...
if(isTRUE(fixStrings)) {
  ...
  # fixStringis is set in the workFlow_init.R script
...
if(NCCAASCIIParse == TRUE) {
  ...
  # activitySpoof <- TRUE
  # forceActivitySpoof <- FALSE
  # activitySpoof and forceActivitySpoof are set in the NCCA_ASCII_init.R script
  
  # PLESpoof <- TRUE
  # forcePLESpoof <- FALSE
  # PLESpoof and forcePLESpoof are set in the NCCA_ASCII_init.R script
  
  print("source the NCCAASCIIParse.R script to parse the data")
  source(paste0(RPath, 'NCCAASCIIParse.R'), echo=FALSE)
...
if(fixSensorNames==TRUE) {
  ...
  # fixSensorNames is initialized in the workFlow_init.R script
...
######## spoof the missing X and XX announcements #########
...
if(isTRUE(spoofXXX)) {
  ...
  # spoofXXX is set in the NCCA_ASCII_init.R script
  
  source(paste0(RPath, 'spoofXXX.R'), echo=FALSE)
  
  spoofXXXFn(uniqueExams=uniqueExams)
...
{ 
  ...
  # previously called during signal processing
...
if(isTRUE(writeNCAAASCII_LAF)) {
  ...
  # set to false so the script does not run unintentionally
...
#######################   extract the measurements to a data frame   #########################
...
# source(paste0(RPath, 'chartPlot_init.R'))
...
# saveResultsToPDF is set in the NCCAASCII_init.R script
