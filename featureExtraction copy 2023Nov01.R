...
featureExtraction <- function(x=uniqueExams, 
  ...
  for(i in 1:length(uniqueExams)) {
    
    {
      ...
      if(showNames==TRUE) print(paste("exam name:", examName))
    ...
    for(j in 1:length(uniqueSeries)) {
      ...
      #### # Feb 4, 2022 initialize a pattern data frame for this series ####
      ...
      for(k in 1:length(uniqueCharts)) {
        ...
        for (l in 1:length(eventNames)) {
          ...
          # if(all(i == 1, seriesName == 2, chartName == "01A" && segmentName == "R5")) {
          #   assign("extract.params", extract.params, pos=1)
          #   assign("chartDF", chartDF, envir=.GlobalEnv)
          #   assign("segmentDF", segmentDF, pos=1)
          #   assign("segmentName", segmentName, pos=1)
          ...
          # if(all(i == 1, seriesName == 2, chartName == "04A" && segmentName == "C10")) {
          #   assign("chartDF", chartDF, envir=.GlobalEnv)
          #   assign("segmentDF", segmentDF, pos=1)
          #   assign("extract.params", extract.params, pos=1)
          #   assign("segmentName", segmentName, pos=1)
          ...
          # if(all(seriesName=="2", chartName=="03A" && segmentName=="C6")) {
          #   assign("segmentDF", segmentDF, pos=1)
          #   assign("extract.params", extract.params, pos=1)
          #   stop()
          # }
        ...
        #### check for extreme unusual values ####
