...
for(i in 1:length(uniqueExams)) {
  ...
  for(j in 1:length(uniqueSeries)) {
    ...
    for(k in 1:length(uniqueCharts)) {
      ...
      #                        scaleVals['uPneumo'] / (max(chartDF$c_UPneumo[(1:nrow(chartDF))]) - min(chartDF$c_UPneumo[(1:nrow(chartDF))])),
      #                        scaleVals['uPneumo'] / (max(chartDF$c_UPneumo[301:600]) - min(chartDF$c_UPneumo[301:600])))
      # LPneumoScale <- ifelse(nrow(chartDF) < 600,
      #                        scaleVals['lPneumo'] / (max(chartDF$c_LPneumo[(1:nrow(chartDF))]) - min(chartDF$c_LPneumo[(1:nrow(chartDF))])),
      #                        scaleVals['lPneumo'] / (max(chartDF$c_LPneumo[301:600]) - min(chartDF$c_LPneumo[301:600]))) 
      ...
      for (l in 1:length(eventNames)) {
        ...
        # get the segment start row and end row for later use
        # startRow <- prestimRow
        ...
        # UPneumoOffset <- segmentDF$c_UPneumo[1]
        # LPneumoOffset <- segmentDF$c_LPneumo[1]
        # AutoEDAOffset <- segmentDF$c_AutoEDA[1]
        # Cardio1Offset <- segmentDF$c_CardioMid[1]
        # if(inclPLE==TRUE) { PLOffset <- segmentDF$c_PLMA[1] }
        # if(activityWarning=="none") {  ActivityOffset <- segmentDF$c_SEMA[1] }
        ...
        # segmentDF$c_UPneumo <- segmentDF$c_UPneumo - UPneumoOffset + yOffset[1]
        # segmentDF$c_LPneumo <- segmentDF$c_LPneumo - LPneumoOffset + yOffset[2]
        # segmentDF$c_UPneumoMid <- segmentDF$c_UPneumoMid - UPneumoOffset + yOffset[1]
        # segmentDF$c_LPneumoMid <- segmentDF$c_LPneumoMid - LPneumoOffset + yOffset[2]
        # segmentDF$c_UPneumoInh <- segmentDF$c_UPneumoInh - UPneumoOffset + yOffset[1]
        # segmentDF$c_LPneumoInh <- segmentDF$c_LPneumoInh - LPneumoOffset + yOffset[2]
        # segmentDF$c_UPneumoExh <- segmentDF$c_UPneumoExh - UPneumoOffset + yOffset[1]
        # segmentDF$c_LPneumoExh <- segmentDF$c_LPneumoExh - LPneumoOffset + yOffset[2]
        
        # segmentDF$c_AutoEDA <- segmentDF$c_AutoEDA - AutoEDAOffset + yOffset[3]
        # segmentDF$c_AutoEDAMid <- segmentDF$c_AutoEDAMid - AutoEDAOffset + yOffset[3]
        # segmentDF$c_AutoEDAPeak <- segmentDF$c_AutoEDAPeak - AutoEDAOffset + yOffset[3]
        # segmentDF$c_AutoEDABase <- segmentDF$c_AutoEDABase - AutoEDAOffset + yOffset[3]
        
        # segmentDF$c_Cardio1 <- segmentDF$c_Cardio1 - Cardio1Offset + yOffset[4]
        # # use the Cardio1Offset to center the CardioMA data on the Cardio1 data
        # segmentDF$c_CardioMid <- segmentDF$c_CardioMid - Cardio1Offset + yOffset[4]
        # segmentDF$c_CardioMA <- segmentDF$c_CardioMA - Cardio1Offset + yOffset[4]
        # segmentDF$c_CardioDiastolic <- segmentDF$c_CardioDiastolic - Cardio1Offset + yOffset[4]
        # segmentDF$c_CardioSystolic <- segmentDF$c_CardioSystolic - Cardio1Offset + yOffset[4]

        # if(inclPLE==TRUE) { 
        #   segmentDF$c_PL <- segmentDF$c_PL - PLOffset + yOffset[5]
        #   segmentDF$c_PLMA <- segmentDF$c_PLMA - PLOffset + yOffset[5]
        #   segmentDF$c_PLMax <- segmentDF$c_PLMax - PLOffset + yOffset[5]
        #   segmentDF$c_PLMin <- segmentDF$c_PLMin - PLOffset + yOffset[5]
        # } # end if for PLE

        # if(activityWarning=="none") {
        #   segmentDF$c_SE <- segmentDF$c_SE - ActivityOffset + yOffset[6]
        #   segmentDF$c_SEMA <- segmentDF$c_SEMA - ActivityOffset + yOffset[6]
        #   segmentDF$c_SEMin <- segmentDF$c_SEMin - ActivityOffset + yOffset[6]
        #   segmentDF$c_SEMax <- segmentDF$c_SEMax - ActivityOffset + yOffset[6]
        # }
        ...
        # if(min(segmentDF$c_Cardio1) < -165) {
        #   newCardio1Offset <- min(segmentDF$c_Cardio1[1:(length(segmentDF$c_Cardio1)-150)]) - 1-65
        #   segmentDF$c_Cardio1 <- segmentDF$c_Cardio1 - newCardio1Offset
        #   segmentDF$c_CardioMid <- segmentDF$c_CardioMid - newCardio1Offset
        #   segmentDF$c_CardioMA <- segmentDF$c_CardioMA - newCardio1Offset
        #   segmentDF$c_CardioDiastolic <- segmentDF$c_CardioDiastolic - newCardio1Offset
        #   segmentDF$c_CardioSystolic <- segmentDF$c_CardioSystolic - newCardio1Offset
        #   cardioWarning <- "UNSTABLE CARDIO DATA"
        # } else cardioWarning <- "none"
        
        # # re-offset the pneumo data to keep the data on the plot
        # if(max(segmentDF$c_UPneumo) > 165) {
        #   newPneumoOffset <- max(segmentDF$c_UPneumo) - 165
        #   segmentDF$c_UPneumo <- segmentDF$c_UPneumo - newPneumoOffset
        #   segmentDF$c_LPneumo <- segmentDF$c_LPneumo - newPneumoOffset
        #   # include the mid inhalation and exhalation trend lines
        #   segmentDF$c_UPneumoMid <- segmentDF$c_UPneumoMid - newPneumoOffset
        #   segmentDF$c_LPneumoMid <- segmentDF$c_LPneumoMid - newPneumoOffset
        #   segmentDF$c_UPneumoInh <- segmentDF$c_UPneumoInh - newPneumoOffset
        #   segmentDF$c_LPneumoInh <- segmentDF$c_LPneumoInh - newPneumoOffset
        #   segmentDF$c_UPneumoExh <- segmentDF$c_UPneumoExh - newPneumoOffset
        #   segmentDF$c_LPneumoExh <- segmentDF$c_LPneumoExh - newPneumoOffset
        # }
        
#        # re-offset the activity sensor data to keep the data on the plot
#        if(activityWarning=="none") {
#           if(min(segmentDF$c_SE) < -175) {
#             newActivityOffset <- min(segmentDF$c_SE) - 175)
#             segmentDF$c_SE <- segmentDF$c_SE + newActivityOffset
#           }
#         }
        ...
        g <- g + geom_line(data=segmentDF, aes(x=(1:nrow(segmentDF)), y=c_UPneumo), color="blue1", size=.25) + coord_cartesian(ylim=c(-175, 175))
        ...
        g <- g + geom_line(data=segmentDF, aes(x=(1:nrow(segmentDF)), y=c_LPneumo), color="blue4", size=.25) + coord_cartesian(ylim=c(-175, 175))
        ...
        if(showArtifacts==TRUE) {
          ...
          g <- g + annotate("point", x=which(segmentDF$UPneumo_a=="Artifact"),
                            y=segmentDF$c_UPneumo[which(segmentDF$UPneumo_a=="Artifact")],
                            shape=4, size=3)
          # g <- g + annotate("point", x=which(segmentDF$UPneumoInh_a=="Artifact"),
          #                   y=segmentDF$c_UPneumo[which(segmentDF$UPneumoInh_a=="Artifact")],
          #                   shape=4, size=3)
          # g <- g + annotate("point", x=which(segmentDF$UPneumoExh_a=="Artifact"),
          #                   y=segmentDF$c_UPneumo[which(segmentDF$UPneumoExh_a=="Artifact")],
          #                   shape=4, size=3)
          # g <- g + annotate("point", x=which(segmentDF$UPneumoMid_a=="Artifact"),
          #                   y=segmentDF$c_UPneumo[which(segmentDF$UPneumoMid_a=="Artifact")],
          #                   shape=4, size=3)
          
          g <- g + annotate("point", x=which(segmentDF$LPneumo_a=="Artifact"),
                            y=segmentDF$c_LPneumo[which(segmentDF$LPneumo_a=="Artifact")],
                            shape=4, size=3)
          # g <- g + annotate("point", x=which(segmentDF$LPneumoInh_a=="Artifact"),
          #                   y=segmentDF$c_LPneumo[which(segmentDF$LPneumoInh_a=="Artifact")],
          #                   shape=4, size=3)
          # g <- g + annotate("point", x=which(segmentDF$LPneumoExh_a=="Artifact"),
          #                   y=segmentDF$c_LPneumo[which(segmentDF$LPneumoExh_a=="Artifact")],
          #                   shape=4, size=3)
          # g <- g + annotate("point", x=which(segmentDF$LPneumoMid_a=="Artifact"),
          #                   y=segmentDF$c_LPneumo[which(segmentDF$LPneumoMid_a=="Artifact")],
          #                   shape=4, size=3)
          ...
          UPArtifactBuffYOn <- segmentDF$c_UPneumo[UPArtifactBuffXOn]
          UPArtifactBuffYOff <- segmentDF$c_UPneumo[UPArtifactBuffXOff]
          ...
          LPArtifactBuffYOn <- segmentDF$c_LPneumo[LPArtifactBuffXOn]
          LPArtifactBuffYOff <- segmentDF$c_LPneumo[LPArtifactBuffXOff]
          g <- g + annotate("segment",
                            x=LPArtifactBuffXOn,
        ...
        if(!(segmentName %in% excludeEvents)) {
          ...
          if(showMeasurements==TRUE) {
            if(!is.na(which(segmentDF$UPneumoExtract=="responseOnsetRow")[1])) {
              g <- g + geom_line(data=segmentDF[onsetRow:segEndRow,], aes(x=onsetRow:segEndRow, y=c_UPneumo), color="blue1", size=.8)
              g <- g + geom_line(data=segmentDF[onsetRow:segEndRow,], aes(x=onsetRow:segEndRow, y=c_LPneumo), color="blue3", size=.8)
          ...
          #     aBuffYOnU <- segmentDF$c_UPneumo[aBuffXOnU][1]
          #     aBuffYOffU <- segmentDF$c_UPneumo[aBuffXOffU][1]
          #     g <- g + annotate("segment",
          #                       x=aBuffXOnU,
          #                       xend=aBuffXOffU,
          #                       y=aBuffYOnU,
          ...
          #     aBuffYOnL <- segmentDF$c_LPneumo[aBuffXOnU][1]
          #     aBuffYOffL <- segmentDF$c_LPneumo[aBuffXOffU][1]
          #     g <- g + annotate("segment",
          #                       x=aBuffXOnL,
          #                       xend=aBuffXOffL,
          #                       y=aBuffYOnL,
