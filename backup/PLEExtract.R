...
PLEExtractFn <- function(x=uniqueExams, showNames=TRUE, output=FALSE) {
  ...
  # source('~/R/NCCA_ASCII_Parse/PLEMeasurement.R', echo=FALSE)
  ########
...
#        excludeEvents <- c("BI", "SW", "X", "XX", "WRQ", "RS", "TI", "EI", "EE", "MV", "MVT", "MI", "CA", "AI", "TDB", "SLP", "WU", "CA", "OS", "OTH", "B", "T", "C", "Y", "BN", "SNF", "CT", "LGH", "DB")
        ...
        for (l in 1:length(eventNames)) {
          ...
          PLEList <- PLEMeasurement(dataVector=segmentDF$c_PL, stimulusOnset=segOnsetRow, cps=30)
          
          # get the prestimMeanAmp and stimResponseMeanAmp measurements
#           prestimMeanAmp <- mean(segmentDF$c_PLAmp[(prestimSegOnset:prestimSegOffset)])
#           stimResponseMeanAmp <- mean(segmentDF$c_PLAmp[(poststimSegOnset:poststimSegOffset)])
          
          # add the means to the segment data frame
#           segmentDF$PLEMeans[(prestimSegOnset:prestimSegOffset)] <- prestimMeanAmp
#           segmentDF$PLEMeans[(poststimSegOnset:poststimSegOffset)] <- stimResponseMeanAmp
          ...
          # and finally calculate the PLE change measurement and add it to the data frame
#           segmentDF$PLEMeasure[(segOnsetRow:segEndRow)] <-
#             rep(stimResponseMeanAmp / prestimMeanAmp, times=(segEndRow-segOnsetRow+1))

#           segmentDF$PLEMeasure[(segOnsetRow:segEndRow)] <- stimResponseMeanAmp
