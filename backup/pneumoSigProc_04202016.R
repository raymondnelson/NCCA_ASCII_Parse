...
#    outputList <- NULL
    ...
    for(j in 1:length(uniqueSeries)) {
      ...
      for(k in 1:length(uniqueCharts)) {
        ...
        chartDF$c_UPneumoMid <- MASmooth(x=chartDF$c_UPneumo, y=round(5*cps,0), times=15) # was y=40, times=8 11/19/2015
        ...
        chartDF$c_LPneumoMid <- MASmooth(x=chartDF$c_LPneumo, y=round(5*cps,0), times=15)
