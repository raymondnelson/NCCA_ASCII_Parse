...
pneumoExtractFn <- function(x=uniqueExams, showNames=TRUE, output=FALSE) {
  ...
  # source('~/R/NCCA_ASCII_Parse/robustPneumoMeasurement.R', echo=FALSE)
  ...
  for(i in 1:length(uniqueExams)) {
    ...
    for(j in 1:length(uniqueSeries)) {
      ...
      for(k in 1:length(uniqueCharts)) {
        ...
        # check the respiration rate for the chart 
        # not used because the respiration rate is now checked for each segment
