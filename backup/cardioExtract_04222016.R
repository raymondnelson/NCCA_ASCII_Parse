...
cardioExtractFn <- function(x=uniqueExams, showNames=TRUE, output=FALSE, EW=FALSE, ROW=FALSE, descentRule=TRUE, ignore=2, cardioLine="mid") {
  ...
  for(i in 1:length(uniqueExams)) {
    ...
    for(j in 1:length(uniqueSeries)) {
      ...
      for(k in 1:length(uniqueCharts)) {
        ...
        # if(round(60 / (mean(diff(maxPeak(chartDF$c_Cardio1, y=8))) / cps), 0) < 60) { 
        #   chartDF$CardioExtract <- "ONR"
        #   seriesDF[chartOnsetRow:(nrow(chartDF)+chartOnsetRow-1),] <- chartDF
        #   next }
        # if(round(60 / (mean(diff(maxPeak(chartDF$c_Cardio1, y=8))) / cps), 0) > 100) { 
        #   chartDF$CardioExtract <- "ONR"
        #   seriesDF[chartOnsetRow:(nrow(chartDF)+chartOnsetRow-1),] <- chartDF
        #   next }
...
# newCardioExtractFn(x=uniqueExams, showNames=TRUE, output=FALSE, strictWindow=FALSE, strictROW=FALSE, descentStop=TRUE, ignore=2)
