...
newEDAExtractFn <- function(x=uniqueExams, showNames=TRUE, output=FALSE, EW=FALSE, ROW=FALSE, descentRule=TRUE) {
...
#           ifelse(length(events)==9,
#                  for(i in 9:5) {if(events[i-1] == events[i]) events[i-1] <- events[i-1] - 1},
#                  for(i in 6:5) {if(events[i-1] == events[i]) events[i-1] <- events[i-1] - 1}
#                  )
...
#           segmentDF$AutoEDAExtract[events["responseOnsetRow"]] <- "responseOnsetRow"
#           segmentDF$AutoEDAExtract[events["responseEndRow"]] <- "responseEndRow"
          # segmentDF$AutoEDAExtract[events["segEndRow"]] <- "endRow"
          # segmentDF$AutoEDAExtract[events["stopRow"]] <- "stopRow"

#           segmentDF$AutoEDAMeasure[segOnsetRow:segEndRow] <- rep(responseChange, times=(segEndRow-segOnsetRow+1))
...
# newEDAExtractFn(x=uniqueExams, showNames=TRUE, output=FALSE, strictWindow=FALSE, strictROW=FALSE, descentStop=TRUE)
