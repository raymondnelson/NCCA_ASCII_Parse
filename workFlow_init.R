...
{  
  ...
  { 
    ...
    # used in the workFlow to call a function
    spoofXXX <- FALSE
    # spoofXXX <- TRUE
    ...
    # activity and PLE spoof parameter is in the NCCAASCII_init.R script
    # because those are called when parsing the NCCAASCII data 
    ...
    # forceActivitySpoof is normally set in the NCCAASCII_init.R script
    # forceActivitySpoof <- TRUE
    
    # used in the data parse script
    PLESpoof <- FALSE
    # PLESpoof <- TRUE
    ...
    # repeated here in case of changes with the above configs
    if(!(PLESpoof)) forcePLESpoof <- FALSE
    if(!(activitySpoof)) forceActivitySpoof <- FALSE
...
#######  define a function to locate exams in the CWD #########
