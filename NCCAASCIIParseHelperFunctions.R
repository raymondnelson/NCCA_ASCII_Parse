...

{
  ...
  # source a script to spoof the activity sensor data 
  source(paste0(RPath, 'activitySpoof.R'), echo=FALSE)
  
  # source a script to spoof the PLE sensor data
  source(paste0(RPath, 'PLESpoof.R'), echo=FALSE)
...
dataParse <- function(x=dataNames, y=thisExamName, saveCSV=FALSE, makeDF=TRUE) {
  ...
  # is called by the parseUniqueExams() function in NCCAASCIIParse.R
  #
  ####
  ...
  
  {
    ...
    # initialize a scalar for the weighting coefs for activity spoof
    weightCoefsAS <- c(sample(c(.2, .25, .3, .35, .4), 1, prob=c(.1, .2, .3, .4, .5)), sample(c(.75, .8, .85, .9, .95), 1, prob=c(.1, .2, .3, .4, .5)))
