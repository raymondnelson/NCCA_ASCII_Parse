...
#
##
# Feb 24, 2022 PlE and RLE constraints
####
...
######### spoof missing sensor data for PLE and activity #########

{
  
  # normally set all to false
  
  # these are used by the data parse function
  # and this is why these parameters are here instead of in the workFlow.R script
  
  # to spoof a missing activity sensor channel 
  # activity spoof is called by the data parse function
  activitySpoof <- FALSE
  activitySpoof <- TRUE
  
  # to coerce activity spoof when activity data are extant
  forceActivitySpoof <- FALSE
  # forceActivitySpoof <- TRUE
  
  if(!(activitySpoof)) forceActivitySpoof <- FALSE
  
  # to spoof a missing PLE channel
  # PLE spoof is called by the data parse function
  PLESpoof <- FALSE
  PLESpoof <- TRUE
  
  # coerce the PLE spoof even when the PLE data are present
  forcePLESpoof <- FALSE
  # forcePLESpoof <- TRUE
  
  if(!(PLESpoof)) forcePLESpoof <- FALSE
  
  # PLEBaseline controls a filter to constraint the tracing to a stable baseline
  PLEBaseline <- FALSE
  PLEBaseline <- TRUE
  
  # X and XX spoof parameter is in the workFlow.R script
